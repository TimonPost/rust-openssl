use libc::{sockaddr_storage,sockaddr_in,sockaddr_in6, sockaddr_nl, c_int};

use super::sa_family_t;
use errno::Errno;
use libc;
use std::{fmt, mem, net, ptr, slice, io};
use std::ffi::OsStr;
use std::hash::{Hash, Hasher};
use std::path::Path;
use std::os::unix::ffi::OsStrExt;
#[cfg(any(target_os = "android", target_os = "linux"))]
use self::netlink::NetlinkAddr;
#[cfg(any(target_os = "android", target_os = "linux"))]
use self::addr::alg::AlgAddr;
#[cfg(any(target_os = "ios", target_os = "macos"))]
use std::os::unix::io::RawFd;
#[cfg(any(target_os = "ios", target_os = "macos"))]
use sys_control::SysControlAddr;
...
use ssl::addr::netlink::NetlinkAddr;
use ssl::addr::alg::AlgAddr;
...
pub unsafe fn sockaddr_storage_to_addr(
    ...
    match c_int::from(addr.ss_family) {
        ...
        libc::AF_UNIX => {
            let sun = *(addr as *const _ as *const sockaddr_in);
